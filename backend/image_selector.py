...
async def select_image_for_syndrome(lesion_site: str) -> Optional[str]:
    ...
    try:
        ...
        prompt = (
            "INSTRUÇÕES DE SELEÇÃO DE IMAGEM (LEIA TUDO):\n"
            "Você deve escolher EXATAMENTE um arquivo .png da LISTA DE ARQUIVOS DISPONÍVEIS que represente a artéria primária e o "
            "local anatômico compatíveis com a síndrome vascular descrita.\n"
            "\n"
            "⚠️ NUNCA retorne uma imagem cuja nomenclatura indique um local anatômico contraditório (ex.: Síndrome de Wallenberg ≠ ponte).\n"
            "Se não houver arquivo coerente, retorne EXATAMENTE a string 'NAO_ENCONTRADA'.\n"
            "\n"
            "ORDEM DE PRIORIDADE PARA A ESCOLHA:\n"
            "1) Nome completo ou abreviação da artéria presente no nome do arquivo;\n"
            "2) Nome da síndrome presente no nome do arquivo;\n"
            "3) Termo do local anatômico (lesion_site) presente no nome do arquivo.\n"
            "\n"
            "DADOS DE CONTEXTO:\n"
            ...
            "• Retorne SOMENTE o nome exato do arquivo escolhido (com extensão .png) OU 'NAO_ENCONTRADA'.\n"
            "• Não inclua comentários, explicações ou formatação extra.\n"
            "\n"
